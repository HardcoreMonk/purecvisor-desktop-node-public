...
- 현재 운영 제품은 `0.42.75-admin-smoke`다. Web Console과 PCVCLI가 active operator
  surface이고 TUI는 ADR-0011에 따라 absent다. 최신 닫힌 manual-admin package-pair는
  `0.42.74-admin-smoke -> 0.42.75-admin-smoke`이며, feature qualification은
  `promotion_eligible=true`다.
- 현재 Required CI 기준은 final `main`
  ...
  `32904006619`는 required check가 아닌 transition residue다.

아래 `0.42.30-admin-smoke` 설명은 2026-05-18 historical predecessor snapshot이다.

- 2026-05-18 당시 product payload package record는 `0.42.30-admin-smoke`다. Package evidence는
...
- 2026-05-18 당시 internal admin-smoke full-gate/current-card anchor는 `0.42.30-admin-smoke`다.
  Full gate evidence는
  ...
  commit은 `f4349cf049db66b0ae1d5d38a948a6b03a8b0648`다. 당시 닫힌 manual-admin package-pair
...
현재 Required CI의 installer/policy 검증은 `.github/workflows/development-gates.yml`의
`installer-policy` .NET verifier shard가 소유한다. final `main`
`6e2bdb93ce308b632c929e2c17f5550ac3845401`, run `32904006595`에서 `dotnet`, `web`,
`delivery`, `installer-policy` 네 required context가 PASS했다.

아래 Pester 명령은 legacy/manual parity용이며 Required CI가 아니다. 동일한 pwsh 기반
Public Boundary run `32904006619`도 non-required transition residue로만 보존한다.
